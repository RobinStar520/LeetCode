...
/*
Approach:
    For this question, we need to find the first different element between the two vectors. If the first difference appears in the last of the vectors,
We can't decide which one has the correct data. If different values appear in the middle of two vectors, then you need to determine the next value of 
this value.
*/
...
public:
    ...
    {
        auto[it1, it2] = std::mismatch(sensor1.begin(), sensor1.end(), sensor2.begin(), sensor2.end());
        // The first different element appearns at the end of vectors, we can't decide which sensor has correct data.
        if(it1 == sensor1.end())
        {
            return -1; 
        }
        if(std::equal(std::next(it1),sensor1.end(),it2,std::prev(sensor2.end())))
        {
            if(std::equal(std::next(it2),sensor2.end(),it1,std::prev(sensor1.end())))
            {
                return -1; 
            }
            return 2; 
