...
public:
    vector<long long> maximumCoins(vector<int>& heroes, vector<int>& monsters, vector<int>& coins) 
    {
        ...
        vector<long long> prefix(coins.size(), 0ll);
        for (int i=0; i<monsters.size(); ++i)
        {
            temp.push_back(make_pair(monsters[i], coins[i]));
        }
        sort(temp.begin(), temp.end());
        for (int i=0; i<coins.size(); ++i)
        {
            coins[i]= temp[i].second;
        }
        ...
        for (const auto& hero: heroes)
        {
            ...
            auto iter = upper_bound(monsters.cbegin(), monsters.cend(), hero);
            if (iter != monsters.cend())
            {
                int index = iter - monsters.cbegin();
                long long totalCoins = (index > 0) ? prefix[index - 1] : 0;
                result.push_back(totalCoins);
            }
            else
            {
